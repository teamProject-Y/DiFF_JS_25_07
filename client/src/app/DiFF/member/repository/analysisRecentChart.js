...
export default function AnalysisRecentChart({ history = [], isMyRepo }) {
    const mode = useTheme();
    ...
    if (!recentHistory.length) {
        return (
            <div className="h-full w-full flex flex-col items-center justify-center text-center text-gray-600 dark:text-neutral-400">
                ...
                <div className="text-lg font-bold">No recent analysis.</div>
                {isMyRepo && (
    ...
    const data = {
        datasets: [
            { label: 'Bugs',               data: recentHistory.map(h => pt(h, 'bugs')),                   borderColor: theme, borderWidth: 1, backgroundColor: 'rgb(255, 99, 132)',   tension: 0.3 },
            { label: 'Code Smells',        data: recentHistory.map(h => pt(h, 'codeSmells')),             borderColor: theme, borderWidth: 1, backgroundColor: 'rgb(255, 159, 64)',   tension: 0.3 },
            { label: 'Complexity',         data: recentHistory.map(h => pt(h, 'complexity')),             borderColor: theme, borderWidth: 1, backgroundColor: 'rgb(255, 205, 86)',   tension: 0.3 },
            { label: 'Coverage (%)',       data: recentHistory.map(h => pt(h, 'coverage')),               borderColor: theme, borderWidth: 1, backgroundColor: 'rgb(34, 197, 94)',    tension: 0.3 },
            { label: 'Duplications (%)',   data: recentHistory.map(h => pt(h, 'duplicatedLinesDensity')), borderColor: theme, borderWidth: 1, backgroundColor: 'rgb(59, 130, 246)',   tension: 0.3 },
            { label: 'Vulnerabilities',    data: recentHistory.map(h => pt(h, 'vulnerabilities')),        borderColor: theme, borderWidth: 1, backgroundColor: 'rgb(168, 85, 247)',   tension: 0.3 },
        ],
    };
