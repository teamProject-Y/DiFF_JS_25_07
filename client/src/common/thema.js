...
export default function ThemeToggle() {
    ...
    useEffect(() => {
        ...
        applyTheme(isDark ? 'dark' : 'light'); // 루트에 반영 + 브로드캐스트
        setDark(isDark);
    }, []);

    const toggleDark = () => {
        const next = document.documentElement.classList.contains('dark') ? 'light' : 'dark';
        applyTheme(next);           // 루트에 반영 + 브로드캐스트
        setDark(next === 'dark');
    };
    ...
    return (
        <label className="swap swap-rotate cursor-pointer">
            ...
            {/* moon */}
            <svg className="swap-on h-8 w-8 fill-current text-gray-300" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
...
function subscribeTheme(callback) {
    ...
    const onMql = () => {
        const saved = localStorage.getItem('theme');
        if (!saved) callback(); // 저장값 없을 때만 시스템 변화 반영
    };
...
export function useTheme() {
    return useSyncExternalStore(subscribeTheme, getThemeNow, () => 'light');
}
